...
- [構文](#syntax)
- [アフォーダンス](#affordances)
    - [アニティの制約](#arity-restrictions)
    - [型の制約](#type-restrictions)
...
# <a name="projects-repository"> プロジェクトのリポジトリ
...
### <a name="arity-restrictions"> アリティの制約
...
### <a name="existential-restrictions"> 存在の制約
...
### <a name="identifier-restrictions"> 識別子の制約
...
### <a name="boundaries-restrictions"> 境界の制約
...
### <a name="user-defined-restrictions"> ユーザー定義の制約
*注：ユーザー定義の制約システムはまだ実験段階です。*

上記の基本的な制約は、プログラムがランタイムエラーに遭遇しないことを少なくとも保証するべきです。
これらの制約は、CXの固有の[進化的アルゴリズム](#integrated-evolutionary-algorithm)などの興味深いシステムを構築するのに十分なものでなければなりません。
...
# <a name="strict-typing-system"> 強い型付けシステム
...
# <a name="compiled-and-interpreted"> コンパイルと逐次実行
...
### <a name="read-eval-print-loop"> Read-Eval-Print Loop　(REPL) 
...
### <a name="meta-programming-commands"> メタプログラミングコマンド
...
### <a name="interactive-debugging"> 対話的なデバッグ
...
# <a name="integrated-evolutionary-algorithm"> 統合された進化的アルゴリズム
...
# <a name="serialization"> シリアライゼーション
