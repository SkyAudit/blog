+++
title = "Obelisk консенсус: алгоритм, дизайн, мотивы"
tags = [
    "Obelisk",
    "Consensus",
    "Skywire",
...
>>>>Затем вы добавляете подпись, используя публичный ключ в адресе А, чтобы подписать сообщение
>>>>в своём Obelisk блокчейне.

>>>>Как вариант, Вы можете опубликовать публичный ключ для адреса A и затем просто
...
>>>>Эта система ограничивает количество Obelisk узлов и ограничивает возможность запуска узлов для
...
>>Мы хэшируем этот ключ в адрес и после этого адрес хранит 10 монет в качестве своих выходных данных.


>>Это не добавляет стоимости. Это подтверждает, что у вас есть 10 монет. Подтверждает, что Вы знаете
>>приватный ключ для публичного ключа адреса, в котором хранится 10 монет. Вы по-прежнему можете транить монеты
...
>>Когда новый Obelisk узел запущен, он будет "деверять" нескольким случайным узлам. Пользователь
...
>>Мы хотим, чтобы сеть была безопасно даже со случайно выбранными узлами. Мы не хотим допустить
>>ситуацию, которая произошла у Ripple, когда 3 узла разработчиков контролируют всю сеть.
>>network. Однако, мы хотели предотвратить ситуацию, когда кто-нибудь запустит
...
>>Возможно, мы сделаем ограничение, что новые пользователи смогут доверять только случайным узлам, на которых
...
>>немного улучшить время достижения консенсуса блоками.  Немного умениться радиус сети.
...
>>Если у двух больших бирж разных консенсус для конкретного блока, то это проблема.
...
У каждого узла есть публичный ключ, который является индентификатором узла.
...
Это двоичный "Ben-Or" и Skycoin будет использовать что-нибудь более продвинутое,
